...
<template>
  <div class="dropdown mr-4" style="float: right; padding: 13px">
    <a
      href="#"
    ...
    <span class="badge badge-danger" v-if="sayi!=0">{{sayi}}</span>
    <ul
      class="dropdown-menu dropdown-menu-center pull-right mt-4"
      role="menu"
      aria-labelledby="dropdownMenu1"
    >
      <li role="presentation">
        <a href="/admin/mesaj" class="dropdown-menu-header" style="color:Black">Tüm Bildirimler</a>
      </li>
      <ul
        class="timeline timeline-icons timeline-sm"
    ...
    font-size: small;
    color:grey;">
           {{item.not_text .slice(0, 10)}}...</span>
            <span class="timeline-icon"
              ><i class="fa fa-user"></i
            ></span>
            <span class="timeline-date">{{item.not_created_at}}</span>
          </span>
        </li>
...
export default {
  ...
  created() {
    axios.get(`http://localhost/api/bell-list`).then((res) => {
      this.sayi = res.data.length;
      this.items=res.data;
    });
  },
  methods:{
